...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'doctor' | 'spray' | 'fakecheck' | 'mandi' | 'outbreak'>('doctor');
  ...
  const handleAnalyze = async () => {
    ...
    if (isSimulatedOffline || !navigator.onLine) {
      ...
      setError('📡 Offline Field Mode: आपका डायग्नोसिस और फोटो सुरक्षित रूप से लोकल कतार (Offline Queue) में सहेज लिया गया है। इंटरनेट कनेक्टिविटी मिलने पर "Offline & Sync" से तुरंत सिंक करें।');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!navigator.onLine || errMsg.includes('Failed to fetch') || errMsg.includes('NetworkError')) {
        try {
          ...
          const existing = saved ? JSON.parse(saved) : [];
          localStorage.setItem('kisan_offline_diagnoses_queue', JSON.stringify([offlineItem, ...existing]));
          errMsg = 'नेटवर्क कनेक्शन अनुपलब्ध है। आपका फोटो और अनुरोध सुरक्षित रूप से "Offline Queue" में सहेज लिया गया है।';
        } catch (_) {}
      ...
      <nav aria-label="Main Navigation" className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-emerald-100 shadow-2xs">
        <div className="max-w-5xl mx-auto px-4 flex items-center justify-between sm:justify-start gap-2 overflow-x-auto py-2.5 scrollbar-none text-xs sm:text-sm font-bold">
          ...
          >
            ...
            <span>Anti-Fake Pesticide (नकली दवा जांच)</span>
          </button>
          ...
          >
            ...
            <span>Outbreak Radar (रोग अलर्ट)</span>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3">
                    ...
                    >
                      ...
                      <div className="font-extrabold text-xs text-stone-900 group-hover:text-emerald-800">
                        {formatBilingual('Precision Spray & Weather', 'sprayCalculator', currentLanguage.code)}
                      </div>
                      <p className="text-[10px] text-stone-500 line-clamp-1">
                        Tank dose & safe spray window
                      </p>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => setActiveTab('fakecheck')}
                      className="p-3.5 rounded-2xl bg-white border border-amber-200 hover:border-amber-600 text-left transition-all hover:shadow-xs group space-y-1.5 cursor-pointer"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => setActiveTab('mandi')}
                      className="p-3.5 rounded-2xl bg-white border border-stone-200 hover:border-emerald-500 text-left transition-all hover:shadow-xs group space-y-1.5 cursor-pointer"
                    >
                      <div className="w-8 h-8 rounded-xl bg-emerald-100 text-emerald-900 flex items-center justify-center font-bold">
